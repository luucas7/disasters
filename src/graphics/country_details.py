...
class CountryDetails:
    ...
    def create_disaster_row(self, disaster_type: str, count: int, percentage: float) -> html.Div:
        ...
        return html.Div([
            html.Div([
                ...
                html.Div([
                    ...
                    html.Span(f" ({percentage:.1f}%)", className="text-sm text-gray-500")
                ])
            ], className="flex justify-between items-center py-2")
        ], className="hover:bg-gray-50 px-4")
        
    def create_details_content(self, country_iso: Optional[str], filtered_data: Optional[pd.DataFrame] = None) -> html.Div:
        ...
        if data_to_use is None:
            return html.Div("No data available", className="text-gray-500 italic text-center p-4")
        ...
        disaster_rows = []
        for disaster_type, count in disaster_counts.sort_values(ascending=False).items():
            percentage = (count/total_disasters)*100
            disaster_rows.append(self.create_disaster_row(disaster_type, count, percentage))

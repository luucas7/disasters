        ...
        return html.Div([
            html.Div([
                self._create_stat_box(
                    ...
                    initial_value=len(self.data) if self.data is not None else 0
                ),
                self._create_stat_box(
                    "Countries Affected",
                    id_prefix="affected-countries",
                    initial_value=self.data['Country'].nunique() if self.data is not None else 0
                ),
                self._create_stat_box(
                    "Total Deaths",
                    id_prefix="total-deaths",
                    initial_value=self.data['Total Deaths'].sum() if self.data is not None else 0
                ),
                self._create_stat_box(
                    "People Affected",
...
def register_statistics_callbacks(app: Any, data: pd.DataFrame) -> None:
    ...
    def update_statistics(start_year: int, end_year: int) -> List[html.Div]:
        ...
        return [
            Statistics._create_stat_box("Total Disasters", "total-disasters", stats['total_disasters']),
            Statistics._create_stat_box("Countries Affected", "affected-countries", stats['countries']),
            Statistics._create_stat_box("Total Deaths", "total-deaths", stats['total_deaths']),
            Statistics._create_stat_box("People Affected", "affected-people", stats['total_affected'])
        ]
...
class Map:
    ...
    def create_figure(self, filtered_data: pd.DataFrame, impact_metric: str = "Density") -> go.Figure:
        ...
        if impact_metric == "Density":
            ...
            counts_by_country["Disaster_Density"] = (
                (counts_by_country["Disaster_Count"]) / counts_by_country["Area"]
            )
            # Apply cube root transformation to amplify small differences
            counts_by_country["Display_Value"] = np.cbrt(
                counts_by_country["Disaster_Density"]
            )
        else:  # Count
            counts_by_country["Display_Value"] = np.log10(
        ...
        counts_by_country["Scaled_Value"] = (
            counts_by_country["Display_Value"] - min_val
        ) / (max_val - min_val)
        ...
        fig = go.Figure(
            go.Choroplethmapbox(
                ...
                customdata=counts_by_country[
                    ["Disaster_Count", "Country", "Area"]
                ].values,
                colorbar=dict(
